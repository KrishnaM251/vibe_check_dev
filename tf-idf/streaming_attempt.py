...
# with open('conv_battle.json', 'r', encoding='utf-8') as file:
#     data = json.load(file)

# Extract text content from conversations
documents = []
with open("mini_conv_battle.json", 'r', encoding='utf-8') as file:
        # Initialize an empty string to accumulate JSON objects
        data = ""
        # Iterate through each line in the file
        for line in file:
            # Accumulate lines until a full JSON object is detected
            data += line.strip()
            print(line.strip())
            if data.endswith("},"):
                print("supposed to load")
                try:
                    # Remove the trailing comma and attempt to load the JSON object
                    item = json.loads(data.rstrip(','))
                    document = ""
                    for conversation in item['conversation_a'] + item['conversation_b']:
                        document += conversation['content'] + " "
                    print(document)  # or handle the result as needed
                    documents.append(document)
                    # Reset data string for the next JSON object
                    data = ""
